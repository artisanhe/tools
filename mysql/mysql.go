...
func (m MySQL) DockerDefaults() conf.DockerDefaults {
	return conf.DockerDefaults{
		"Host": "db-master", //conf.RancherInternal("tool-dbs", m.Name),
		"Port": 33306,
	}
}
...
func (l *logger) Print(values ...interface{}) {
	if len(values) > 1 {
		...
		if level == "sql" {
			...
			messages = append(messages, color.RedString(sqlRegexp.ReplaceAllString(values[3].(string), "%v"), formatedValues...))
			// duration
			messages = append(messages, color.MagentaString(" [%fms]", float64(values[2].(time.Duration).Nanoseconds()/1e4)/100.0))
		} else {
