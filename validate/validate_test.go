...
import (
	...
	"github.com/artisanhe/tools/validate/validatetpl"
)
...
func TestValidateItemWithPredefineTag(t *testing.T) {
	...
	value_not_num_string := string("1234 ")
	if ok, str := ValidateItem("@numString[0,]", value_not_num_string); ok {
		t.Error("should be invalid")
	} else {
		t.Log(str)
	}
...
func TestNumStringRange(t *testing.T) {
	numString := string("123456")
	ok, str := ValidateItem("@numString[10,20]", numString)
	assert.False(t, ok)
	assert.Equal(t, "字符串长度不在[10， 20]范围内，当前长度：6", str)
}
