...
import (
	...
	"github.com/artisanhe/tools/courier/client/gen/common"
...
func (g *TypeGenerator) Type(schema *oas.Schema) (string, bool) {
	pointer := ""
	if schema != nil && schema.Extensions[gen.XPointer] != nil {
...
func (g *TypeGenerator) TypeIndirect(schema *oas.Schema) (string, bool) {
	...
	if schema.Refer.RefString() != "" {
		return common.RefName(schema.Refer.RefString()), true
	}

	if schema.Extensions[gen.XNamed] != nil {
		if schema.Type == "array" && schema.Items != nil && schema.Items.Format == "uint64" {
			return g.Importer.Use("github.com/artisanhe/tools/httplib.Uint64List"), true
		}

		typeFullName := fmt.Sprint(schema.Extensions[gen.XNamed])
		isInCommonLib := strings.Contains(typeFullName, "github.com/artisanhe/tools")
		...
		if schema.Type == "string" || schema.Type == "boolean" || schema.Enum != nil || isInCommonLib {
			...
			if schema.Type == "boolean" {
				typeName = "Bool"
				pkgImportName = "github.com/artisanhe/tools/courier/enumeration"
				isInCommonLib = true
			}
...
`)
		...
		for _, subSchema := range schema.AllOf {
			if subSchema.Refer.RefString() != "" {
				field := common.NewField(common.RefName(subSchema.Refer.RefString()))
