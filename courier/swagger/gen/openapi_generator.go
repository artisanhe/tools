...
func (g *SwaggerGenerator) getOperationByOperatorTypes(method string, operatorTypes ...*OperatorTypeName) *oas.Operation {
	operators := make([]Operator, 0)

	for _, operatorType := range operatorTypes {
		operators = append(operators, *g.operatorScanner.Operator(operatorType.TypeName))
	}
