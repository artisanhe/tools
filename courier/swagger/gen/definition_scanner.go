...
func Package(program *loader.Program, path string) *loader.PackageInfo {
	for _, info := range program.AllPackages {
		pkgPath := info.Pkg.Path()
		if path == pkgPath || path == codegen.DeVendor(pkgPath) || codegen.DeVendor(path) == codegen.DeVendor(pkgPath) {
			return info
		}
	}
	for _, info := range program.Created {
		pkgPath := info.Pkg.Path()
		if path == pkgPath || path == codegen.DeVendor(pkgPath) || codegen.DeVendor(path) == codegen.DeVendor(pkgPath) {
			return info
		}
...
func (scanner *DefinitionScanner) getSchemaByTypeString(typeString string) *oas.Schema {
	...
	pkgImportPath = loaderx.ResolvePkgImport(pkgImportPath)
	pkgInfo := Package(scanner.program, pkgImportPath)
	if pkgInfo == nil {
		panic(fmt.Errorf("missing pkg %s of %s", pkgImportPath, typeString))
	}
...
func (scanner *DefinitionScanner) getSchemaByType(tpe types.Type) *oas.Schema {
	...
	case *types.Array:
		...
		return oas.ItemsOf(scanner.getSchemaByType(typArray.Elem())).WithValidation(&oas.SchemaValidation{
			MaxItems: ptr.Uint64(uint64(length)),
			MinItems: ptr.Uint64(uint64(length)),
		})
	case *types.Struct:
		...
		for i := 0; i < structType.NumFields(); i++ {
			...
			if jsonTagValue == "" {
				jsonTagValue = structFieldTags.Get("name")
			}
