...
func (m *Enum) WriteInitFunc(w io.Writer) {
	io.WriteString(w, `
func init () {
	`+m.Importer.Use("github.com/artisanhe/tools/courier/enumeration.RegisterEnums")+`("`+m.Name+`", map[string]string{
`)
...
`)
	...
	for _, option := range m.Options {
		if m.HasOffset {
			io.WriteString(w, fmt.Sprintf(`int(%s) + int(%s): {%s, %s},
`, m.ConstKey(option.Value), m.ConstOffset(), strconv.Quote(option.Value.(string)), strconv.Quote(option.Label)))
		} else {
...
func (v *`+m.Name+`) Scan(src interface{}) error {
	integer, err := `+m.Importer.Use("github.com/artisanhe/tools/courier/enumeration.AsInt64")+`(src, `+m.ConstOffset()+`)
	if err != nil {
		return err
	}
