...
func init() {
	git_chinawayltd_com_golib_tools_courier_enumeration.RegisterEnums("State", map[string]string{
		"ACTIVE": "激活",
		"CLOSED": "关闭",
	})
}
...
func (State) Enums() map[int][]string {
	return map[int][]string{
		int(STATE__ACTIVE) + int(STATE_OFFSET): {"ACTIVE", "激活"},
		int(STATE__CLOSED) + int(STATE_OFFSET): {"CLOSED", "关闭"},
	}
}
...
func (v *State) Scan(src interface{}) error {
	integer, err := git_chinawayltd_com_golib_tools_courier_enumeration.AsInt64(src, STATE_OFFSET)
	if err != nil {
		return err
	}
